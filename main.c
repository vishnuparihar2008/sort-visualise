//   1 / 2 / 3  – switch algorithm (reshuffles array)
//   R          – reshuffle
...
typedef struct {
    ...
    int  i, j;       /* indices being highlighted               */
    bool done;
} SortStatus;
...
static void swapEl(int a, int b) {
    int t = numbers[a]; numbers[a] = numbers[b]; numbers[b] = t;
}

static void shuffleNumbers(void) {
    for (int i = 0; i < COUNT; i++) numbers[i] = i + 1;   /* 1-based so no zero-height bar */
...
static void drawBars(SortStatus s, const char *name, int spf) {
    ...
    int   maxH  = baseY - 40;          /* leave room for HUD at top */
    ...
    /* HUD – bottom */
    const char *hint = s.done
        ? "Done!   R=reshuffle   1/2/3=algorithm"
...
static void bubbleReset(void) { bs_i = 0; bs_anySwap = false; }
...
static void selectionReset(void) { ss_i = 0; ss_j = 1; ss_min = 0; }
...
static SortStatus selectionStep(void) {
    ...
    if (ss_j < COUNT) {
        ...
        s.i = ss_min; s.j = ss_j;
        s.comparing = true;
        if (numbers[ss_j] < numbers[ss_min]) ss_min = ss_j;
        ss_j++;
    } else {
...
static void insertionReset(void) { is_i = 1; is_j = 1; }
...
static SortStatus insertionStep(void) {
    ...
    if (is_j > 0 && numbers[is_j - 1] > numbers[is_j]) {
        ...
        /* element is in place; move to next unsorted element */
        is_i++;
        is_j = is_i;
    }
    return s;
...
/* ── algorithm table  (add new algorithms here) ──────────────────────────── */
...
int main(int argc, char *argv[]) {
    ...
    while (!WindowShouldClose()) {
        ...
        for (int k = 0; k < ALGO_COUNT; k++) {
            if (IsKeyPressed(KEY_ONE + k)) {    /* KEY_ONE, KEY_TWO, KEY_THREE are sequential */
                algoIdx = k;
                shuffleNumbers();
                algos[algoIdx].reset();
                status = INIT_STATUS;
        ...
        if (IsKeyPressed(KEY_UP)   && stepsPerFrame < COUNT) stepsPerFrame++;
